...
renderer.setClearColor(0x000000, 0); // شفافية تامة
// خلي الكانڤاس ثابت وخلف المحتوى لكن فوق خلفية الـ body
renderer.domElement.style.position = "fixed";
(renderer.domElement.style as any).inset = "0";
...
const material = new THREE.ShaderMaterial({
  ...
  fragmentShader: `
    ...
    float fbm(vec2 x){ float v=0.0; float a=0.3; vec2 shift=vec2(100.0);
      mat2 rot=mat2(cos(0.5),sin(0.5),-sin(0.5),cos(0.5));
      for(int i=0;i<NUM_OCTAVES;++i){ v+=a*noise(x); x=rot*x*2.0+shift; a*=0.4; } return v; }
    void main(){
      vec2 shake=vec2(sin(iTime*1.2)*0.005, cos(iTime*2.1)*0.005);
      vec2 p=((gl_FragCoord.xy + shake*iResolution.xy) - iResolution*0.5)/iResolution.y * mat2(6.0,-4.0,4.0,6.0);
      ...
      for(float i=0.0;i<35.0;i++){
        ...
        vec4 aur=vec4(0.1+0.3*sin(i*0.2+iTime*0.4),
                      0.3+0.5*cos(i*0.3+iTime*0.5),
                      0.7+0.3*sin(i*0.4+iTime*0.3), 1.0);
        vec4 contrib = aur * exp(sin(i*i + iTime*0.8))
                      / length(max(v, vec2(v.x*f*0.015, v.y*1.5)));
        float thin = smoothstep(0.0,1.0, i/35.0)*0.6;
        o += contrib * (1.0 + tail*0.8) * thin;
      }
      o = tanh(pow(o/100.0, vec4(1.6)));
      gl_FragColor = o * 1.9; // سطوع أعلى
    }
  `,
});
...
// z-0 هنا، وباقي المحتوى سنرفعه z-10
